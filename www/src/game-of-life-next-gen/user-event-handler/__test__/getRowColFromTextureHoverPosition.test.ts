...
describe('getRowColFromTextureHoverPosition', () => {
  it('should return correct row and column for the center of texture', () => {
    ...
    expect(result.row).toEqual(Math.floor(0.5 * DEFAULT_FIELD_SIZE));
    expect(result.col).toEqual(Math.floor(0.5 * DEFAULT_FIELD_SIZE));
  });

  it('should return correct row and column for the first cell position', () => {
    const halfSizeOfCell = TEXTURE_SIZE / DEFAULT_FIELD_SIZE / 2;
  ...
  it('should return correct row and column for the last cell position', () => {
    ...
    expect(result.row).toEqual(Math.floor(DEFAULT_FIELD_SIZE - 1));
    expect(result.col).toEqual(Math.floor(DEFAULT_FIELD_SIZE - 1));
  });
