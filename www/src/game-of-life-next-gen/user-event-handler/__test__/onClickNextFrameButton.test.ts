...
describe('onClickNextFrameButton', () => {
  ...
  it('ticks the universe, draws grid, and draws cells', () => {
    ...
    onClickNextFrameButton(universe, memory, updateTextureContext, width, height, lifeSpan);

    // Expectations
    expect(universe.tick).toHaveBeenCalledWith(true); // Ensure tick method is called with true
    expect(drawGrid).toHaveBeenCalledWith(updateTextureContext, width, height); // Ensure drawGrid is called with correct arguments
    expect(drawCells).toHaveBeenCalledWith(universe, memory, updateTextureContext, width, height, lifeSpan); // Ensure drawCells is called with correct arguments
  });
});
